...
class TestAddImageToPost(TestCase):
    ...
    def setUp(self):
        ...
        self.group = Group.objects.create(
            title=self.group_name, slug=self.group_name
        )
        ...
        with open('media/test_img.jpg', 'rb') as fp:
            response = self.client.post(
                '/new/',
                {'group': self.group.id, 'text': self.text, 'image': fp},
            )
    ...
    def test_index_img(self):
        """
        проверяют, что на главной странице пост с картинкой отображается
    ...
    def test_profile_img(self):
        """
        проверяют, что на странице профайла пост с картинкой отображается
        корректно, с тегом <img>
        """
    ...
    def test_group_img(self):
        """
        проверяют, что на странице группы пост с картинкой отображается
        корректно, с тегом <img>
        """
    ...
    def test_format_non_img_defence(self):
        """
        проверяют, что срабатывает защита от загрузки файлов не-графических
        форматов
        """
...
class TestUserScripts(TestCase):
    ...
    def test_send_email(self):
        """
        Пользователь регистрируется и ему отправляется письмо
    ...
    def test_new_post_publication_profile(self):
        """
        После публикации поста новая запись появляется на персональной
        странице пользователя (profile)
        """
        response = self.client.get(f'/{self.username}/')
        self.assertContains(response, self.text)
    ...
    def test_edit_post(self):
        ...
        и его содержимое изменится
        """
        edited_text = 'Test post text (edited).'
        with open('media/test_img.jpg', 'rb') as fp:
            response = self.client.post(
                f'/{self.username}/{self.post.id}/edit/',
                {'text': edited_text, 'image': fp},
            )
